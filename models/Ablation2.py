...
class Model(nn.Module):
    ...
    def __init__(self, configs, **kwargs):
        ...
        self.patchmamba1 = PatchModel(self.c_in, self.pred_len, self.patch_len, configs.d_state, self.d_model, configs.dropout, configs.factor, configs.n_heads, model='mamba', flatten='d')
        self.patchmamba2 = PatchModel(self.c_in, self.pred_len, self.patch_len, configs.d_state, self.d_model, configs.dropout, configs.factor, configs.n_heads, model='mamba', flatten='p')
    ...
    def forward(self, batch_x, batch_x_mark, batch_y, batch_y_mark):
        ...
        x = self.patchmamba2(x_enc)
        out = x.permute(0, 2, 1)[:, :, :N]
        out = self.revin(out, 'denorm')

        #Test2 Attention + Patch
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = x_enc + self.dropout(self.attention(self.channel_p, x_enc, x_enc, attn_mask=None)[0])
        # x = self.norm(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        # out = self.revin(out, 'denorm')
        ...
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = x_enc + self.dropout(self.mamba1(x_enc) + self.mamba2(x_enc.flip(dims=[-1])).flip(dims=[-1]))
        # x = self.norm(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        # out = self.revin(out, 'denorm')

        # #Test4 Mamba + Attention + Patch
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = x_enc + self.dropout(self.mamba1(x_enc) + self.mamba2(x_enc.flip(dims=[-1])).flip(dims=[-1]))
        # x = self.norm1(x)
        # x = x + self.dropout(self.attention(self.channel_p, x, x, attn_mask=None)[0])
        # x = self.norm2(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        # out = self.revin(out, 'denorm')

        # #Test5  Attention + Mamba + Patch
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = x_enc + self.dropout(self.attention(self.channel_p, x_enc, x_enc, attn_mask=None)[0])
        # x = self.norm1(x)
        # x = x + self.dropout(self.mamba1(x) + self.mamba2(x.flip(dims=[-1])).flip(dims=[-1]))
        # x = self.norm2(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        # out = self.revin(out, 'denorm')

        # #Test6  Attention & Mamba & Patch
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        ...
        # #Test7 Patch + Attention & Mamba 
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = self.patchmamba1(x_enc)
        # x1 = x + self.dropout(self.attention(self.channel_p, x, x, attn_mask=None)[0])
        # x1 = self.norm1(x1)
        # x2 = x + self.dropout(self.mamba1(x) + self.mamba2(x.flip(dims=[-1])).flip(dims=[-1]))
        # x2 = self.norm2(x2)
        # new_x = x1 + x2
        # x = new_x + x
        # x = self.norm(x)
        ...
        # #Test8 Attention & Mamba + Patch (Gate)
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x1 = x_enc + self.dropout(self.attention(self.channel_p, x_enc, x_enc, attn_mask=None)[0])
        ...
        # x = torch.sigmoid(gate_conv) * x1 + (1 - torch.sigmoid(gate_conv)) * x2
        # x = x_enc + x
        # x = self.norm(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        ...
        # #Test9 Attention & Mamba + Patch (Attention)
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x1 = x_enc + self.dropout(self.attention(self.channel_p, x_enc, x_enc, attn_mask=None)[0])
        ...
        # x2 = x_enc + self.dropout(self.mamba1(x_enc) + self.mamba2(x_enc.flip(dims=[-1])).flip(dims=[-1]))
        # x2 = self.norm2(x2)
        # x = self.attention_m(x1, x2, x2, attn_mask=None)[0]
        # x = x_enc + x
        # x = self.norm(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        ...
        # #Test10 Attention & Mamba + Patch (Add&Norm)
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x1 = x_enc + self.dropout(self.attention(self.channel_p, x_enc, x_enc, attn_mask=None)[0])
        ...
        #Test11 Attention(no_c) & Mamba + Patch (Gate)
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x1 = x_enc + self.dropout(self.attention(x_enc, x_enc, x_enc, attn_mask=None)[0])
        # x1 = self.norm1(x1)
        # x2 = x_enc + self.dropout(self.mamba1(x_enc) + self.mamba2(x_enc.flip(dims=[-1])).flip(dims=[-1]))
        # x2 = self.norm2(x2)
        # gate_conv = self.gate_conv(torch.cat([x1, x2],dim=-1).permute(0, 2, 1)).permute(0, 2, 1)
        ...
        #Test12 Original-Attention + Patch
        # batch_x = self.revin(batch_x, 'norm')
        # _,_,N = batch_x.shape
        # x_enc = self.enc_embedding_i(batch_x, batch_x_mark)
        # x = x_enc + self.dropout(self.attention(x_enc, x_enc, x_enc, attn_mask=None)[0])
        # x = self.norm(x)
        # x = self.patchmamba2(x)
        # out = x.permute(0, 2, 1)[:, :, :N]
        # out = self.revin(out, 'denorm')
